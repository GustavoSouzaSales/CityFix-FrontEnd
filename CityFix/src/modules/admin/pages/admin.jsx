...
function Admin() {
  ...
  function salvarNovoUsuario(e) {
    ...
    setNovoUsuario({ nome: "", email: "", telefone: "", cidade: "", tipo: "Usuário" });
    setModalNovoUsuario(false);
  }

  const statsData = [
    { emoji: "📋", label: "Denúncias abertas", valor: denuncias.filter((d) => d.status === "Aberta").length,       cor: "stat--green"  },
    { emoji: "⏳", label: "Em andamento",       valor: denuncias.filter((d) => d.status === "Em andamento").length, cor: "stat--yellow" },
                    ...
                    <article className="admin-list-item" key={denuncia.titulo}>
                      <div className="list-item-info">
                        <div className="list-item-title-row">
                          ...
                          <span className={`prioridade-badge ${prioridadeConfig[denuncia.prioridade]?.cor}`}>
                            {denuncia.prioridade}
                          </span>
                        </div>
                        <p className="list-item-local">📍 {denuncia.local}</p>
                        <p className="list-item-meta">{denuncia.autor} · {denuncia.categoria}</p>
                      </div>
                      <div className="admin-item-actions">
                        <span className={`status-badge ${statusConfig[denuncia.status]?.cor}`}>
                          <span
                            className="status-dot"
                            style={{ background: statusConfig[denuncia.status]?.dot }}
                          />
                          {denuncia.status}
                        </span>
                        <button
                          className="btn-outline"
                          onClick={() => { setDenunciaSelecionada(denuncia); setModalDetalhes(true); }}
                        >
                          Detalhes
                        </button>
                        <button
                          className="btn-solid"
                          onClick={() => { setDenunciaSelecionada(denuncia); setModalAtualizar(true); }}
                        >
                          Atualizar
                        </button>
                      </div>
            ...
            {modalAberto === "usuarios" && (
              <div className="admin-modal-content">
                <div className="admin-filter-row">
                  ...
                  <button className="new-user-btn" onClick={() => setModalNovoUsuario(true)}>
                    ＋ Novo usuário
                  </button>
                </div>
                    ...
                    <article className="admin-list-item" key={usuario.email}>
                      <div className="list-item-info">
                        <div className="usuario-avatar-row">
                          ...
                          <div>
                            <h3>{usuario.nome}</h3>
                            <p className="list-item-meta">{usuario.email} · {usuario.cidade}</p>
                          </div>
                        </div>
                      </div>
                      <div className="admin-item-actions">
                        <span className={`tipo-badge ${usuario.tipo === "Administrador" ? "tipo--admin" : "tipo--user"}`}>
                          {usuario.tipo}
                        </span>
                        <span className="denuncias-count">{usuario.denuncias} denúncias</span>
                        <button
                          className="btn-solid"
                          onClick={() => { setUsuarioSelecionado(usuario); setModalPerfilUsuario(true); }}
                        >
                          Ver perfil
                        </button>
                      </div>
            ...
            <div className="update-form">
              <div className="modal-form-group">
                ...
                <select
                  ...
                  onChange={(e) =>
                    setDenunciaSelecionada({ ...denunciaSelecionada, status: e.target.value })
                  }
                >
                  <option>Aberta</option>
                  <option>Em andamento</option>
              ...
              <div className="modal-form-group">
                ...
                <select
                  ...
                  onChange={(e) =>
                    setDenunciaSelecionada({ ...denunciaSelecionada, prioridade: e.target.value })
                  }
                >
                  <option>Baixa</option>
                  <option>Média</option>
              ...
              <button
                ...
                onClick={() => {
                  setDenuncias(denuncias.map((d) =>
                    d.titulo === denunciaSelecionada.titulo ? denunciaSelecionada : d
                  ));
              ...
              {[
                ...
                <div className="modal-form-group" key={key}>
                  ...
                  <input
                    ...
                    onChange={(e) => setNovoUsuario({ ...novoUsuario, [key]: e.target.value })}
                  />
                </div>
              ))}
              <div className="modal-form-group">
          ...
          <div className="admin-modal details-modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="perfil-usuario-topo">
              <div className="usuario-avatar large">
                {usuarioSelecionado.nome.split(" ").map((n) => n[0]).join("").slice(0, 2).toUpperCase()}
              </div>
              <div>
                <h3>{usuarioSelecionado.nome}</h3>
                <span className={`tipo-badge ${usuarioSelecionado.tipo === "Administrador" ? "tipo--admin" : "tipo--user"}`}>
